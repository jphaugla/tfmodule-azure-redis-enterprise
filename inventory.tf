resource "local_file" "instances_file" {
    ...
    content = templatefile("../templates/inventory.tpl",
        {
            ...
            cassandra_ips = "${join("\n",(azurerm_linux_virtual_machine.cassandra-node.*.public_ip_address) )}"
    ...
    depends_on = [
        azurerm_public_ip.fixedip, azurerm_linux_virtual_machine.tester-node, azurerm_linux_virtual_machine.kafka-node, azurerm_linux_virtual_machine.cassandra-node
    ]
}
